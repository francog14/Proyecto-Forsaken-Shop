...
@Component({
  ...
  templateUrl: './productos-cliente.html',
...
export class ProductosClienteComponent implements OnInit {
  ...
  readonly availableSizes = ['S', 'M', 'L', 'XL'];

  readonly availableColors = computed(() => {
    const colors = new Set(this.prendas().map(p => p.color));
    return Array.from(colors).sort();
  });

  readonly precioMin = computed(() => {
    const precios = this.prendas().map(p => p.precio_prenda);
  ...
  readonly filteredPrendas = computed(() => {
    let result = this.prendas();
    const size = this.selectedSize();
    const color = this.selectedColor();

    if (size) {
      result = result.filter(p => p.talla.toUpperCase() === size.toUpperCase());
    }
    if (color) {
  ...
  cargarCatalogo() {
    ...
    forkJoin({
      ...
      error: () => {
        this.error.set('No se pudo cargar el catálogo de productos.');
      },
    });
  }
  ...
  toggleColor(color: string) {
    this.selectedColor.set(this.selectedColor() === color ? '' : color);
  }
  ...
  textoEstadoPrenda(prenda: Prenda) {
    ...
    if (estado === 'poco-stock') return 'Poco stock';
    return 'Disponible';
  }
  ...
  agregarAlCarrito(prenda: Prenda) {
    ...
    } else if (this.cartService.error()) {
      this.error.set(this.cartService.error());
